...
class bourkestreetbakery: UIViewController {

    @IBOutlet weak var outImg: UIImageView!
    @IBOutlet weak var outImagen2: UIImageView!
    ...
    func pickerView(pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        if component == 0 {
            outImg.image = UIImage(named: Ingredientes1[row])
            indice1 = row
            
        } else {
            //strMensa2 = Ingredientes2[row]
            outImagen2.image = UIImage(named: Ingredientes2[row])
            indice2 = row
        }
